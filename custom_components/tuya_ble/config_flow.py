...
# Hardcoded Netherlands/EU settings
NL_ENDPOINT = "https://openapi.tuyaeu.com"
NL_COUNTRY_CODE = "31"
...
async def _try_login(
    ...
    """Try to login with different combinations of auth types and app types."""
    response: dict[Any, Any] | None = None
    data: dict[str, Any]

    # Base configuration
    data = {
        CONF_ENDPOINT: NL_ENDPOINT,
        CONF_ACCESS_ID: user_input[CONF_ACCESS_ID],
        CONF_ACCESS_SECRET: user_input[CONF_ACCESS_SECRET],
        CONF_USERNAME: user_input[CONF_USERNAME],
        CONF_PASSWORD: user_input[CONF_PASSWORD],
        CONF_COUNTRY_CODE: NL_COUNTRY_CODE,
        CONF_APP_TYPE: SMARTLIFE_APP,  # Default to Smart Life
    }

    # Try different auth types with Smart Life first
    auth_types = [
        AuthType.SMART_HOME,  # Try Smart Home auth first
        AuthType.CUSTOM,      # Then try Custom auth
    ]
    ...
    for auth_type in auth_types:
        ...
        _LOGGER.debug(
            "Trying login with auth_type: %s, app_type: %s",
            auth_type,
            SMARTLIFE_APP,
        )
        ...
        if response.get(TUYA_RESPONSE_SUCCESS, False):
            _LOGGER.debug(
                "Login successful with auth_type: %s, app_type: %s",
                auth_type,
                SMARTLIFE_APP,
            )
            return data
    ...
    if response:
        ...
        _LOGGER.debug(
            "Login failed with code: %s, message: %s",
...
def _show_login_form(
    ...
    schema = {
        ...
        vol.Required(
            CONF_ACCESS_SECRET, default=user_input.get(CONF_ACCESS_SECRET, "")
        ): str,
        vol.Required(
